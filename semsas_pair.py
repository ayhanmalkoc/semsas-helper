#!/usr/bin/env python3
"""semsas-pair — print one QR for Semsas setup.
...
4. Print one Semsas setup JSON QR. Scanning that single QR in Semsas
...
def hermes_models_endpoint_ok(base_url: str, api_key: Optional[str], timeout: float = 4.0) -> bool:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as response:
            ...
            return isinstance(payload, dict) and payload.get("object") == "list" and isinstance(payload.get("data"), list)
    except Exception:
        return False
...
def dashboard_api_ok(local_url: str, token: str, timeout: float = 2.0) -> bool:
    req = urllib.request.Request(local_url.rstrip("/") + "/api/cron/jobs")
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as response:
            ...
            return isinstance(payload, list) or isinstance(payload, dict) and isinstance(payload.get("jobs"), list)
    except Exception:
        return False


def discover_dashboard_terminal(local_url: str = "http://127.0.0.1:9119") -> Optional[dict]:
    """Return verified Hermes dashboard pairing data when `dashboard --tui` is running."""
    try:
    ...
    if not dashboard_api_ok(local_url, token):
        return None
    return {"url": normalize_url(local_url), "token": token}


def parse_dashboard_ports_from_text(text: str) -> List[int]:
    ports: List[int] = []
    patterns = (
        r"(?:^|\s)--port(?:=|\s+)(\d{2,5})(?:\s|$)",
        r"(?:^|\s)--dashboard-port(?:=|\s+)(\d{2,5})(?:\s|$)",
        r"(?:^|\s)HERMES_DASHBOARD_PORT=(\d{2,5})(?:\s|$)",
        r"(?:^|\s)HERMES_TERMINAL_PORT=(\d{2,5})(?:\s|$)",
        r"(?:^|\s)HERMES_MANAGEMENT_PORT=(\d{2,5})(?:\s|$)",
    )
...
def discover_dashboard_process_ports() -> List[int]:
    out = run(["ps", "-eo", "command"], timeout=3) or ""
    ports: List[int] = []
    for line in out.splitlines():
        lowered = line.lower()
        if "hermes" not in lowered or "dashboard" not in lowered:
            continue
        for port in parse_dashboard_ports_from_text(line):
            if port not in ports:
                ports.append(port)
    return ports


def discover_dashboard_docker_ports() -> List[int]:
    if not shutil.which("docker"):
        return []
    out = run(["docker", "ps", "--format", "{{.Names}} {{.Image}} {{.Command}} {{.Ports}}"], timeout=4) or ""
    ports: List[int] = []
    for line in out.splitlines():
        lowered = line.lower()
        ...
        for port in parse_dashboard_ports_from_text(line):
            if port not in ports:
                ports.append(port)
        for match in re.finditer(r"0\.0\.0\.0:(\d{2,5})->\d+/tcp|:::(\d{2,5})->\d+/tcp", line):
...
def discover_dashboard_systemd_ports() -> List[int]:
    if not shutil.which("systemctl"):
        return []
    units = run(["systemctl", "list-units", "--type=service", "--all", "--no-legend", "--plain"], timeout=4) or ""
    ports: List[int] = []
    for line in units.splitlines():
        name = line.split(None, 1)[0] if line.split(None, 1) else ""
        if "hermes" not in name.lower():
            continue
        show = run(["systemctl", "show", name, "-p", "ExecStart", "-p", "Environment"], timeout=4) or ""
        for port in parse_dashboard_ports_from_text(show):
            if port not in ports:
                ports.append(port)
    return ports


def discover_dashboard_evidence_ports() -> List[int]:
    envs = [read_env_file(path) for path in readable_existing(HERMES_ENV_PATHS)]
    ports: List[int] = []
    port_value = env_lookup(("HERMES_DASHBOARD_PORT", "HERMES_TERMINAL_PORT", "HERMES_MANAGEMENT_PORT"), envs) or json_lookup(
        HERMES_JSON_PATHS,
        {"dashboardPort", "dashboard_port", "terminalPort", "terminal_port", "managementPort", "management_port"},
    )
    if port_value and str(port_value).isdigit():
        port = int(str(port_value))
        if 0 < port <= 65535:
            ports.append(port)
    for source_ports in (discover_dashboard_process_ports(), discover_dashboard_docker_ports(), discover_dashboard_systemd_ports()):
...
def discover_dashboard_candidates(origins: Iterable[str] = ()) -> List[str]:
    envs = [read_env_file(path) for path in readable_existing(HERMES_ENV_PATHS)]
    candidates: List[str] = []
    url = env_lookup(("HERMES_DASHBOARD_URL", "HERMES_TERMINAL_URL", "HERMES_MANAGEMENT_URL"), envs) or json_lookup(
        HERMES_JSON_PATHS,
        {"dashboardUrl", "dashboard_url", "terminalUrl", "terminal_url", "managementUrl", "management_url"},
    )
    if url:
        candidates.append(normalize_url(url))
    ports = discover_dashboard_evidence_ports()
    for port in ports:
        candidates.append(f"http://127.0.0.1:{port}")
    for origin in origins:
        try:
            parsed = urllib.parse.urlparse(normalize_url(origin))
        except Exception:
            continue
        ...
        for port in ports:
            candidates.append(f"{scheme}://{parsed.hostname}:{port}")
    return list(dict.fromkeys(candidates))


def discover_any_dashboard_terminal(origins: Iterable[str] = ()) -> Optional[dict]:
    for candidate in discover_dashboard_candidates(origins):
        pairing = discover_dashboard_terminal(candidate)
        if pairing:
            return pairing
    return None
...
def start_terminal_command_server(no_tailscale: bool = False, no_lan: bool = False) -> Optional[dict]:
...
ThreadingHTTPServer(("0.0.0.0", port), TerminalHandler).serve_forever()
'''.lstrip()
    )
    ...
    while time.monotonic() < deadline:
        ...
        log_file.close()
        return None
    host = first_remote_host(no_tailscale, no_lan)
    result = {"port": port, "secret": secret}
    if host:
        result["url"] = f"http://{host}:{port}/run"
    return result
...
def maybe_configure_hermes_remote_access(hermes_key: Optional[str], remote_host: Optional[str]) -> Optional[str]:
    ...
    print(f"To use Semsas from your phone, Hermes must listen on Tailscale/LAN: http://{remote_host}:8642")
    ...
    if wait_for_hermes_remote(remote_host, key):
        ...
        print(
            ...
            "Check firewall/VPN and run semsas-pair again.",
            file=sys.stderr,
        )
    return key
...
def discover_hermes_model() -> Optional[str]:
    envs = [read_env_file(path) for path in readable_existing(HERMES_ENV_PATHS)]
    return env_lookup(("HERMES_MODEL", "HERMES_MODEL_NAME", "API_SERVER_MODEL_NAME", "AGENT_VOICE_HERMES_MODEL"), envs) or json_lookup(
        HERMES_JSON_PATHS,
...
def discover_openclaw_url() -> Optional[str]:
    envs = [read_env_file(path) for path in readable_existing(OPENCLAW_ENV_PATHS)]
    value = env_lookup(
        (
            "OPENCLAW_URL",
            ...
            "GATEWAY_URL",
            "AGENT_VOICE_OPENCLAW_URL",
        ),
        envs,
    ) or json_lookup(
        OPENCLAW_JSON_PATHS,
        {"publicUrl", "public_url", "url", "gatewayUrl", "gateway_url"},
    )
    if value:
        return normalize_url(value)
    if shutil.which("openclaw"):
        port = run(["openclaw", "config", "get", "gateway.port"], timeout=3)
...
def normalize_url(raw: str) -> str:
    ...
    if raw.startswith("wss://"):
        raw = "https://" + raw.removeprefix("wss://")
    elif raw.startswith("ws://"):
        raw = "http://" + raw.removeprefix("ws://")
...
def setup_code_url(code: Optional[str]) -> Optional[str]:
    payload = decode_setup_code(code or "")
    if not payload:
        return None
...
def local_listening_ports() -> List[int]:
    ports: List[int] = []
    try:
        proc = subprocess.run(["ss", "-ltn"], text=True, capture_output=True, timeout=2)
    except Exception:
        return ports
    if proc.returncode != 0:
        return ports
    for match in re.finditer(r":(\d+)\s", proc.stdout):
        try:
            port = int(match.group(1))
        except ValueError:
            continue
        if port > 1024 and port not in ports:
            ports.append(port)
    return ports

def local_listening_endpoints() -> List[tuple[str, int]]:
    ...
    for line in proc.stdout.splitlines():
        ...
        if len(parts) < 4 or parts[0] != "LISTEN":
            continue
        local = parts[3]
        match = re.search(r"(?P<host>.+):(\d+)$", local)
        if not match:
            continue
        try:
...
def discover_local_http_url(path: str, validator, ports: Optional[List[int]] = None) -> Optional[str]:
    for port in ports or local_listening_ports():
        url = f"http://127.0.0.1:{port}"
        try:
            if validator(url.rstrip("/") + path):
                return url
        except Exception:
            continue
    return None

def discover_bound_hermes_url(api_key: Optional[str], no_tailscale: bool, no_lan: bool) -> Optional[str]:
    allowed_hosts = {"0.0.0.0", "::", "*", "127.0.0.1", "localhost"}
    if not no_tailscale:
        for host in [tailscale_ip(), tailscale_dns_name()]:
            if host:
    ...
    for host, port in local_listening_endpoints():
        # Do not classify every OpenAI-compatible router as Hermes. The
        # automatic network probe is intentionally limited to Hermes' API
        # server default port; custom ports must come from config/env or
        # explicit --hermes-url.
        if port != 8642:
            continue
        ...
        if host in {"0.0.0.0", "::", "*", "127.0.0.1", "localhost"}:
            phone_host = first_remote_host(no_tailscale, no_lan) if host in {"0.0.0.0", "::", "*", "127.0.0.1", "localhost"} else host
        ...
        if hermes_models_endpoint_ok(probe_url, api_key, timeout=1.5):
            return f"http://{phone_host}:{port}"
    return None

def first_remote_host(no_tailscale: bool, no_lan: bool) -> Optional[str]:
    if not no_tailscale:
        for host in [tailscale_dns_name(), tailscale_ip()]:
            if host:
                return host
...
def rewrite_local_url_for_phone(url: Optional[str], mode: str, no_tailscale: bool, no_lan: bool) -> Optional[str]:
    if not url:
        return None
    normalized = normalize_url(url)
    if not normalized or mode == "local" or not is_loopback_url(normalized):
        return normalized
    parsed = urllib.parse.urlparse(normalized)
    host = first_remote_host(no_tailscale, no_lan)
    if not host:
        return normalized
    return urllib.parse.urlunparse(parsed._replace(netloc=f"{host}:{parsed.port or 80}"))


def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(prog="semsas-pair", description=__doc__.splitlines()[0])
    p.add_argument("--mode", choices=("remote", "local", "manual"), default="remote", help="Pairing mode: remote server, local phone/proot host, or manual prompts.")
    p.add_argument("--url", "--hermes-url", dest="url", action="append", default=[], help="Hermes URL. Repeat or comma-separate for LAN/Tailscale/public.")
    ...
    p.add_argument("--openclaw-url", help="OpenClaw gateway URL. Overrides the URL embedded in the setup code.")
    ...
    p.add_argument("--no-public-tunnel", action="store_true", help="Do not start temporary public tunnels. This is the default unless --public-tunnel is set.")
    p.add_argument(
        "--tunnel-provider",
    ...
    probed_hermes_url = None if discovered_hermes_url else discover_local_http_url(
        "/v1/models",
        lambda url: hermes_models_endpoint_ok(url.rsplit("/v1/models", 1)[0], hermes_key, timeout=2.0),
    )
    bound_hermes_url = None if (discovered_hermes_url or probed_hermes_url) else discover_bound_hermes_url(
        hermes_key,
        args.no_tailscale,
        args.no_lan,
    )
    dashboard_origins = [url for url in (args.url, discovered_hermes_url, probed_hermes_url, bound_hermes_url) if url]
    dashboard_pairing_probe = discover_any_dashboard_terminal(dashboard_origins)
    dashboard_port_open = dashboard_pairing_probe is not None
    ...
    discovered_openclaw_url = args.openclaw_url or discover_openclaw_url() or setup_code_url(discovered_openclaw_setup_code)

    print("Semsas pairing helper")
    print(f"  Mode:     {args.mode}")
    print(f"  Hermes:   {'found' if hermes_installed else 'not found'}; API {'detected' if (hermes_port_open or discovered_hermes_url or probed_hermes_url or bound_hermes_url) else 'not detected'}; dashboard {'verified' if dashboard_port_open else 'not detected'}")
    print(f"  OpenClaw: {'found' if openclaw_installed else 'not found'}; gateway {'detected' if (openclaw_port_open or discovered_openclaw_url) else 'not detected'}")
    ...
    elif probed_hermes_url:
        print(f"  Hermes URL: probed {probed_hermes_url}")
    elif bound_hermes_url:
        print(f"  Hermes URL: probed {bound_hermes_url}")
    if discovered_openclaw_url:
        print(f"  OpenClaw URL: detected {normalize_url(discovered_openclaw_url)}")
    if hermes_key:
    ...
    auto_hermes = bool(args.url or discovered_hermes_url or probed_hermes_url or bound_hermes_url or hermes_key or hermes_installed or hermes_port_open)
    auto_openclaw = bool(discovered_openclaw_setup_code or discovered_openclaw_url or openclaw_installed or openclaw_port_open)
    if args.hermes_only:
        include_hermes = True
    ...
    terminal_pairing: Optional[dict] = dashboard_pairing_probe if include_hermes and dashboard_port_open else None
    terminal_command_pairing: Optional[dict] = start_terminal_command_server(args.no_tailscale, args.no_lan) if include_openclaw and openclaw_installed else None
    ...
    if use_public_tunnel:
        ...
        if not mux_public_url and include_hermes and terminal_pairing and dashboard_port_open:
            dashboard_public_url = start_public_tunnel(terminal_pairing["url"], "hermes-dashboard", args.tunnel_provider)
        ...
        openclaw_tunnel_source = discovered_openclaw_url or ("http://127.0.0.1:18789" if openclaw_port_open else None)
        if not mux_public_url and include_openclaw and openclaw_tunnel_source:
            openclaw_public_url = start_public_tunnel(openclaw_tunnel_source, "openclaw", args.tunnel_provider)
    ...
    if include_hermes:
        ...
        if discovered_hermes_url and not hermes_public_url:
            unique_append(hermes_candidates, rewrite_local_url_for_phone(discovered_hermes_url, args.mode, args.no_tailscale, args.no_lan))
        if probed_hermes_url and not hermes_public_url:
            unique_append(hermes_candidates, rewrite_local_url_for_phone(probed_hermes_url, args.mode, args.no_tailscale, args.no_lan))
        if bound_hermes_url and not hermes_public_url:
            unique_append(hermes_candidates, bound_hermes_url)
        if not hermes_candidates:
            if hermes_installed:
                unique_append(hermes_candidates, rewrite_local_url_for_phone("http://127.0.0.1:8642", args.mode, args.no_tailscale, args.no_lan))
            elif args.interactive:
    ...
    if include_openclaw:
        ...
        if openclaw_public_url and openclaw_setup_code and wait_for_http_url(openclaw_public_url.rstrip("/") + "/health"):
            ...
            openclaw_setup_code = None
        elif discovered_openclaw_url and openclaw_setup_code:
            phone_openclaw_url = rewrite_local_url_for_phone(discovered_openclaw_url, args.mode, args.no_tailscale, args.no_lan)
            if phone_openclaw_url:
                openclaw_setup_code = setup_code_with_url(openclaw_setup_code, phone_openclaw_url)
    ...
    if not include_hermes and not include_openclaw:
        raise SystemExit(
            ...
            "Start Hermes/OpenClaw or run `semsas-pair --mode manual` to enter values manually."
        )
    ...
    qr_path = write_qr_png(qr_payload, Path(tempfile.gettempdir()) / "semsas-pair-qr.png")
    write_qr_svg(qr_payload, Path(tempfile.gettempdir()) / "semsas-pair-qr.svg")
    ...
    print("\nScan this one QR inside Semsas:")
    ...
    print("\nQR payload for Semsas in-app scanner:")
    print(f"  {qr_payload}\n")
